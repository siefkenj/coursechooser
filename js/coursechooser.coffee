...
$(document).ready ->
    ...
    $('#toggleEdge').click ->
        ...
        if not (svgManager and svgManager.selected[0] and svgManager.selected[1])
            ...
            onPreviewPageShow()
            # after the preview-page update, the svgManager has been changed
            #TODO nasty hack!!
            updateSelected = ->
                svgManager = window.courseManager.svgManager
                svgManager.select($("##{elm1.replace(' ','-')}")[0])
                svgManager.select($("##{elm2.replace(' ','-')}")[0])
    ...
    initializeSVGManager: (svgManager) ->
        ...
        @svgManager.selectionChanged = ->
            if not (svgManager.selected[0] and svgManager.selected[1])
                ...
                edge = edge || window.courseManager.graphState.edges[[course2,course1]]
            $('#edge-selected .course1').html course1?.$elm || ''
            $('#edge-selected .course2').html course2?.$elm || ''
            if not edge
                $('#edge-selected .existing-edge').hide()
            else
                $('#edge-selected .existing-edge').show()
