...
var Course, CourseManager, DiGraph, courseDataLoaded, hideCoursesFromDep, populateYearTable, showCoursesFromDep;
...
hideCoursesFromDep = function(dep) {
  ...
  for (hash in _ref) {
    course = _ref[hash];
    if (course.department === dep && course.state.required === false && course.state.elective === false) {
      if (course.elm) {
  ...
  for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
    ...
    if (!elm.course.bound) {
      ...
      _results.push($(elm).click(function(evt) {
        ...
        return $("#dot").val(CourseManager.createDotGraph());
      }));
    } else {
      _results.push(void 0);
    }
...
CourseManager = {
  ...
  updatePrereqTags: function() {
    var c, hash, prereqs, selected, unmet, _i, _len, _ref, _ref1;
  ...
  createDotGraph: function() {
    var allCourses, allCoursesHash, c, course, courseHashLookup, e, edges, elms, g, hash, i, p, prereqs, ret, years, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _m, _n, _o, _p, _ref, _ref1, _ref2;
    years = {
      1: [],
      2: [],
      3: [],
    ...
    ret = "digraph x {\n";
    for (_n = 0, _len5 = edges.length; _n < _len5; _n++) {
      e = edges[_n];
      ret += "\t\"" + e[0] + "\" -> \"" + e[1] + "\"\n";
    }
    ret += "\n";
...
Course = (function() {
  ...
  function Course(data) {
    ...
    _ref = this.data, this.department = _ref.department, this.number = _ref.number, this.prereqs = _ref.prereqs;
    this.elm = null;
    this.state = {
      required: false,
      elective: false,
  ...
  Course.prototype.getButton = function() {
    ...
    this.$elm = $("<div class='course'><div class='annotation'></div><div class='number'>" + this.department + " " + this.number + "</div></div>");
    this.elm = this.$elm[0];
  ...
  Course.prototype.setState = function(state) {
    var f, k, s, _i, _len, _ref, _ref1, _ref2;
    if (state == null) {
      state = {};
    }
    for (k in state) {
  ...
  Course.prototype.showCourseInfo = function(infoarea) {
    var $infoarea;
    $infoarea = $(infoarea);
    $infoarea.find('.course-name').html("" + this.hash + " &mdash; " + this.data.desc);
    $infoarea.find('.prereq-area').html(Course.prereqsToDivs(this.prereqs));
    this.unbindRadioButtons(infoarea);
    return this.bindRadioButtons(infoarea);
  };
  ...
  Course.prototype.bindRadioButtons = function(infoarea) {
    ...
    changeToggle = function() {
      ...
      if (_this.state.required) {
        return $(infoarea).find('input[value=required]').attr('checked', true);
      } else if (_this.state.elective) {
        return $(infoarea).find('input[value=elective]').attr('checked', true);
      } else {
        return $(infoarea).find('input[value=none]').attr('checked', true);
      }
    };
  ...
  Course.prereqsToDivs = function(prereq) {
    ...
    if (prereq.op) {
      if (typeof prereq.op === 'string') {
        return "<ul class='prereq-tree'><li class='prereq-tree'>" + ((function() {
          var _i, _len, _ref, _results;
          _ref = prereq.data;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            p = _ref[_i];
            _results.push(Course.prereqsToDivs(p));
          }
          return _results;
        })()).join("</li> " + prereq.op + " <li class='prereq-tree'>") + "</ul>";
      } else {
        return "";
      }
    }
  ...
  Course.flattenPrereqs = function(prereq) {
    ...
    if (prereq != null ? prereq.department : void 0) {
      return [prereq];
    }
...
DiGraph = (function() {
  ...
  DiGraph.prototype.toDot = function(name) {
