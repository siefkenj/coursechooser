...
DiGraph = (function() {
  ...
  DiGraph.prototype.breakIntoTerms = function(list, maxTerms) {
    var c, forwardNeighbors, getForwardNeighbordsOfRank, giveNodesOfRank, i, nodes, ranks, ret, updateRanks, _i, _j, _k, _len, _ref,
      _this = this;
    if (maxTerms == null) {
      maxTerms = 3;
    }
    this._generateForwardNeighborHash();
    ranks = {};
    updateRanks = function(updateHash) {
      var k;
      for (k in ranks) {
        if (updateHash[k]) {
          ranks[k] += 1;
    ...
    getForwardNeighbordsOfRank = function(rank) {
      var c, forwardNeighbors, n, _i, _j, _len, _len1, _ref, _ref1;
      forwardNeighbors = {};
      _ref = giveNodesOfRank(rank);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        c = _ref[_i];
        _ref1 = _this.forwardNeighborHash[c] || [];
        for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
          n = _ref1[_j];
    ...
    for (i = _j = 0, _ref = maxTerms - 1; 0 <= _ref ? _j < _ref : _j > _ref; i = 0 <= _ref ? ++_j : --_j) {
      forwardNeighbors = getForwardNeighbordsOfRank(i);
      updateRanks(forwardNeighbors);
    }
    ret = [];
    for (i = _k = 0; 0 <= maxTerms ? _k <= maxTerms : _k >= maxTerms; i = 0 <= maxTerms ? ++_k : --_k) {
  ...
  DiGraph.prototype.toDot = function(name, titles, clusters, ops) {
    var c, clust, course, createAnonymousSubgraph, e, edge, elective, electives, electivesNodes, graph, hash, i, labelWidth, marker, node, prevMarker, rankSubgraph, subgraph, term, termClust, termSubgraph, termSubgraphs, title, year, yearSubgraph, yearSubgraphs, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _len7, _m, _n, _o, _p, _q, _r, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _s, _t, _u, _v;
    if (titles == null) {
      titles = {};
    }
    if (clusters == null) {
    ...
    for (year = _m = 1; _m <= 4; year = ++_m) {
      _ref1 = this.years[year];
      for (_n = 0, _len2 = _ref1.length; _n < _len2; _n++) {
        course = _ref1[_n];
        graph.nodes[course] = {
          attrs: {
            _name: course,
    ...
    for (year = _o = 1; _o <= 4; year = ++_o) {
      _ref2 = this.breakIntoTerms((function() {
        var _len3, _q, _ref2, _results;
        _ref2 = this.years[year];
        _results = [];
        for (_q = 0, _len3 = _ref2.length; _q < _len3; _q++) {
          c = _ref2[_q];
          _results.push(c.hash);
        }
        return _results;
      }).call(this));
      for (i = _p = 0, _len3 = _ref2.length; _p < _len3; i = ++_p) {
        termClust = _ref2[i];
        rankSubgraph = termSubgraphs[year][i + 1];
        for (_q = 0, _len4 = termClust.length; _q < _len4; _q++) {
          course = termClust[_q];
          rankSubgraph.nodes[course] = true;
        }
      }
    }
    for (_r = 0, _len5 = electivesNodes.length; _r < _len5; _r++) {
      elective = electivesNodes[_r];
      subgraph = termSubgraphs[elective.year][1];
    ...
    for (i = _s = 0, _len6 = electives.length; _s < _len6; i = ++_s) {
      ...
      _ref3 = elective.nodes;
      for (_t = 0, _len7 = _ref3.length; _t < _len7; _t++) {
        course = _ref3[_t];
        subgraph.nodes[course] = true;
    ...
    prevMarker = null;
    for (year = _u = 1; _u <= 4; year = ++_u) {
      for (term = _v = 1; _v <= 3; term = ++_v) {
    ...
    for (hash in _ref4) {
      node = _ref4[hash];
      if ((_ref5 = node.attrs['style']) != null ? _ref5.match(/invis/) : void 0) {
        node.attrs['label'] = '';
        continue;
      }
...
Graph = (function() {
  ...
  Graph.prototype.toJSON = function() {
    var c, cluster, edge, node, ret, _, _ref, _ref1, _ref2;
    ret = {
      nodes: [],
      edges: [],
      clusters: []
  ...
  Graph.prototype.toDot = function() {
    var cluster, course, courses, createAnonymousSubgraph, edge, elective, graph, hash, i, index, labelWidth, levels, list, marker, mat, node, prevMarker, rankSubgraph, subgraph, term, termClust, termSubgraph, termSubgraphs, year, yearSubgraph, yearSubgraphs, _, _i, _j, _k, _l, _len, _len1, _len2, _m, _n, _o, _p, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
    createAnonymousSubgraph = function(parent) {
      var subgraph;
      subgraph = graph.addSubgraph(null, parent);
      subgraph.attrs['rank'] = 'same';
    ...
    for (year = _k = 1; _k <= 4; year = ++_k) {
      _ref2 = this._generateAdjacencyMatrix({
        filterByYear: year
      }), mat = _ref2.mat, list = _ref2.list;
